define(["app/core/app_events","app/core/core_module","app/plugins/sdk"], function(__WEBPACK_EXTERNAL_MODULE_grafana_app_core_app_events__, __WEBPACK_EXTERNAL_MODULE_grafana_app_core_core_module__, __WEBPACK_EXTERNAL_MODULE_grafana_app_plugins_sdk__) { return /******/ (function(modules) { // webpackBootstrap
/******/ 	// The module cache
...
var SettingActionInAdvancePageCtrl = /** @class */ (function () {
    ...
    SettingActionInAdvancePageCtrl.prototype.updateIPList = function (id) {
        var _this = this;
    ...
    SettingActionInAdvancePageCtrl.prototype.setMode = function (mode) {
        this.mode = mode;
    };
...
var InspectionPropertyPageCtrl = /** @class */ (function () {
    ...
    InspectionPropertyPageCtrl.prototype.updateInspectionPropertyList = function (selectId) {
        ...
        this.$q.all([this.$q(function (resolve, reject) {
                var query = ["select * from t_inspection_property where IP_TYPE=1"];
                _this.rsDsSrv.query(selectId, query).then(function (result) {
                    var data = _this.rsDsSrv.getTableObj(result);
                    _this.inspectList = data[0];
                    resolve(data);
                }).catch(function (err) {
                    reject(err);
                    console.error(err);
                ...
                var query = ["select * from t_inspection_property where IP_TYPE=2"];
                _this.rsDsSrv.query(selectId, query).then(function (result) {
                    var data = _this.rsDsSrv.getTableObj(result);
                    _this.faultyList = data[0];
                    resolve(data);
                }).catch(function (err) {
                    reject(err);
                    console.error(err);
    ...
    InspectionPropertyPageCtrl.prototype.addInspectionItem = function (ipType, name, desc, min, max, cpk_min, cpk_max) {
        ...
        if (cpk_min !== undefined && cpk_min !== null) {
            values = values + ", " + cpk_min;
        }
        else {
            values = values + ", 0";
        }
        columns = columns + ", DEFAULT_CPK_MAX";
        if (cpk_max !== undefined && cpk_max !== null) {
            values = values + ", " + cpk_max;
        }
        else {
            values = values + ", 0";
        }
    ...
    InspectionPropertyPageCtrl.prototype.deleteInspectionItem = function () {
        var _this = this;
        ...
        this.$rootScope.appEvent('confirm-modal', {
            ...
            yesText: '삭제',
            onConfirm: function () {
                var selectId = _this.appModel.jsonData.datasourceID;
                var query = [];
                ...
                _this.rsDsSrv.query(selectId, query).then(function (result) {
                    ...
                    _this.alertSrv.set(item.NAME + " 삭제 실패", err, 'error', 5000);
                    console.error(err);
                });
            }
        });
    };
    InspectionPropertyPageCtrl.prototype.updateInspectionItem = function () {
        var _this = this;
        var selectId = this.appModel.jsonData.datasourceID;
        var obj = this.selectObj;
        var query = [];
        var updateData = "NAME = '" + obj.NAME + "'"
            + ", DESCRIPTION = '" + obj.DESCRIPTION + "'"
    ...
    InspectionPropertyPageCtrl.prototype.setMode = function (mode) {
        this.mode = mode;
        this.updateInspectionPropertyList(this.appModel.jsonData.datasourceID);
    };
    InspectionPropertyPageCtrl.prototype.showEtcMenu = function (obj) {
        this.selectObj = obj;
        this.enEtcMenu = true;
    };
...
/*!**************************************!*\
  !*** ./services/remoteSolutionWS.ts ***!
  \**************************************/
/*! exports provided: RemoteSolutionWSCtrl */
/***/ (function(module, __webpack_exports__, __webpack_require__) {

"use strict";
__webpack_require__.r(__webpack_exports__);
/* harmony export (binding) */ __webpack_require__.d(__webpack_exports__, "RemoteSolutionWSCtrl", function() { return RemoteSolutionWSCtrl; });
/* harmony import */ var grafana_app_core_core_module__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(/*! grafana/app/core/core_module */ "grafana/app/core/core_module");
/* harmony import */ var grafana_app_core_core_module__WEBPACK_IMPORTED_MODULE_0___default = /*#__PURE__*/__webpack_require__.n(grafana_app_core_core_module__WEBPACK_IMPORTED_MODULE_0__);
...
var RemoteSolutionWSCtrl = /** @class */ (function () {
    ...
    RemoteSolutionWSCtrl.prototype.CreateRemoteSolutionWebsocket = function (appId, urlPath) {
        var _this = this;
        if (undefined === this.wsInfo[appId]) {
            console.log("[rsWsSrv] New Websocket Instance:", appId);
            var url = "ws://" + this.$location.host() + ":" + this.$location.port()
                + "/api/plugin-proxy/" + appId + urlPath;
            this.wsInfo[appId] = {};
            this.wsInfo[appId].conn = new WebSocket(url);
            // register websocket Events
            this.wsInfo[appId].conn.onopen = function (evt) { _this.onWsOpen(evt, appId); };
            this.wsInfo[appId].conn.onclose = function (evt) { _this.onWsClose(evt, appId, urlPath); };
            this.wsInfo[appId].conn.onmessage = function (evt) { _this.onWsMessage(evt, appId); };
            return true;
        }
    ...
    //===================================================================================
    RemoteSolutionWSCtrl.prototype.onWsOpen = function (event, appId) {
        console.log("[rsWsSrv] Websocket Connected:", appId);
        this.RegisterWebsocketClient(appId);
    };
    RemoteSolutionWSCtrl.prototype.onWsClose = function (event, appId, urlPath) {
        var _this = this;
        console.log("[rsWsSrv] Websocket Disconnected:", appId);
        setTimeout(function () {
            // re create websocket
            delete _this.wsInfo[appId];
            _this.CreateRemoteSolutionWebsocket(appId, urlPath);
        }, 2000);
    };
    // Remote Solution Websocket Response Commands
    RemoteSolutionWSCtrl.prototype.onWsMessage = function (event, appId) {
        ...
        // resp_DeviceList
        grafana_app_core_app_events__WEBPACK_IMPORTED_MODULE_1___default.a.emit('ws-recv-data-' + appId, obj);
    };
    //===================================================================================
    // Remote Solution Websocket Reqeust Commands
    //===================================================================================
    RemoteSolutionWSCtrl.prototype.RegisterWebsocketClient = function (appId) {
    ...
    */
    RemoteSolutionWSCtrl.prototype.ReqMessage = function (appId, reqMsg) {
        if (this.wsInfo[appId].conn === undefined) {
            console.error("[rsWsSrv]", appId, "Websocket Instance Not found");
            return false;
        }
    ...
    //===================================================================================
    RemoteSolutionWSCtrl.prototype.getSessionId = function (appId) {
        return this.wsInfo[appId].id;
    };
    return RemoteSolutionWSCtrl;
}());

grafana_app_core_core_module__WEBPACK_IMPORTED_MODULE_0___default.a.service('rsWsSrv', RemoteSolutionWSCtrl);
