...
class RmsProductFailPanelCtrl extends MetricsPanelCtrl {
  ...
  columnOption(obj) {
    ...
    if (count !== -1) {
      ...
      if (obj.title === PRODUCT_TOTAL || obj.title === PRODUCT_SUCESS || obj.title === PRODUCT_FAIL) {
  ...
  transformer(dataList) {
    ...
    tableMap.forEach(function (value, key, mapObj) {
      ...
      value.forEach((v,k) => {
        object[k] = v;
        switch (k) {
          case PRODUCT_SUCESS: tempProduct = v; break;
  ...
  transAddedData(data, tableMap) {
    ...
    if (columns.map(x => x.text).indexOf(PRODUCT_TOTAL) !== -1
      || columns.map(x => x.text).indexOf(PRODUCT_SUCESS) !== -1
      || columns.map(x => x.text).indexOf(PRODUCT_FAIL) !== -1) {
      if(!this.columninit) {
        this.initColume();
